...
export default function ResourceDetail() {
  ...
  // Toggle the "auto-generated" flag inline. Used after the pastor has
  // reviewed an auto-extracted resource and "claimed" it as their own.
  // We clear the auto_source_label too so the provenance breadcrumb
  // doesn't linger on a now-claimed resource.
  const clearAutoGenerated = async () => {
    if (!resource) return;
    setError(null);
    try {
      const { error: err } = await withTimeout(
        supabase
          .from('resources')
          .update({ auto_generated: false, auto_source_label: null })
          .eq('id', resource.id)
      );
      if (err) throw err;
      setResource((r) =>
        r ? { ...r, auto_generated: false, auto_source_label: null } : r
      );
    } catch (e) {
      setError(e.message || String(e));
    }
              ...
              <div className="min-w-0 flex-1">
                <div className="flex flex-wrap items-baseline gap-2">
                  ...
                  {resource.auto_generated && (
                    <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-purple-100 text-purple-800 text-[10px] uppercase tracking-wide">
                      ✨ Auto-generated{' '}
                      {resource.auto_source_label && (
                        <span
                          className="normal-case tracking-normal text-purple-700"
                          title={`From: ${resource.auto_source_label}`}
                        >
                          · {resource.auto_source_label}
                        </span>
                      )}
                      <button
                        ...
                        title="Clear the auto-generated tag — claim this as your own"
                      >
                        clear
                      </button>
                    </span>
