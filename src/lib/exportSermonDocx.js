...
function buildHeader(prefs, ctx) {
  // Pass ctx.page='' so the {page} token (if any) is silently stripped
  // from the rendered header text — instead of being replaced with the
  // preview-only '#' placeholder. The actual page number is rendered
  // by Word itself via a separate field paragraph (see buildFooter).
  const text = renderTokens(prefs.header_content, { ...ctx, page: '' });
  if (!text) {
...
function buildFooter(prefs, ctx) {
  ...
  // Same as buildHeader: strip {page} from the footer text. The page
  // number lives in its own paragraph below, using a real Word field.
  const text = renderTokens(prefs.footer_content, { ...ctx, page: '' });
  if (text) {
    paragraphs.push(
      new Paragraph({
  ...
  if (pos && pos !== 'none' && pos.startsWith('bottom_')) {
    ...
    // Blank paragraph between the footer text and the page number,
    // so the page number visually sits on its own line with breathing
    // room. Only inserted if there's actually footer text above it —
    // otherwise the page number floats at the top of the footer area
    // by itself.
